...
use vuio::database::{binary_format::BinaryMediaFileSerializer, MediaFile};

/// Benchmark comparing custom binary format
#[tokio::test]
...
async fn binary_format_benchmark() -> Result<()> {
    ...
    let total_throughput = test_files.len() as f64
        / (binary_serialize_time.as_secs_f64() + binary_deserialize_time.as_secs_f64());

    println!(
        "⚡ Total throughput:        {:.0} files/sec",
        total_throughput
    );
    println!(
        "📦 Data efficiency:        {:.1} bytes/file",
        binary_data.len() as f64 / test_files.len() as f64
    );
    println!(
        "🎯 Round-trip time:        {:.2}ms",
        (binary_serialize_time + binary_deserialize_time).as_millis()
    );
    ...
    println!("{}", "-".repeat(40));
    println!(
        "Custom Binary: {} bytes total, {:.1} bytes/file",
        binary_data.len(),
        binary_data.len() as f64 / test_files.len() as f64
    );
...
async fn micro_benchmark() -> Result<()> {
    ...
    println!(
        ...
        binary_time.as_micros() as f64 / iterations as f64
    );
    println!(
        "⚡ Operations per sec: {:.0}",
        iterations as f64 / binary_time.as_secs_f64()
    );

    println!(
        "\n🏆 Custom Binary format performance: {:.0} operations/sec!",
        iterations as f64 / binary_time.as_secs_f64()
    );
