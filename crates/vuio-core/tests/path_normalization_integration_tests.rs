...
use std::path::Path;
use vuio_core::platform::filesystem::{
    create_platform_path_normalizer, FileSystemManager, PathNormalizer, WindowsPathNormalizer,
};
...
mod path_normalization_integration_tests {
    ...
    fn test_windows_path_variations() {
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(
                result.unwrap(),
                expected,
                "Path normalization mismatch for: {}",
                input
            );
        }
    }
    ...
    fn test_unc_path_variations() {
        ...
        for (input, expected) in unc_test_cases {
            ...
            assert_eq!(
                result.unwrap(),
                expected,
                "UNC path normalization mismatch for: {}",
                input
            );
        }
    }
    ...
    fn test_extended_length_path_prefix() {
        ...
        let extended_test_cases = vec![
            ...
            (r"\\?\c:\users\media", "c:/users/media"),
            (
                r"\\?\D:\Very\Long\Path\To\Media\Files",
                "d:/very/long/path/to/media/files",
            ),
        ...
        for (input, _expected) in extended_test_cases {
            ...
            match result {
                Ok(canonical) => {
                    println!(
                        "Extended-length path normalized: {} -> {}",
                        input, canonical
                    );
                ...
                Err(e) => {
                    println!(
                        "Extended-length path normalization failed (may be expected): {} -> {:?}",
                        input, e
                    );
    ...
    fn test_unicode_character_handling() {
        ...
        for (input, expected) in basic_unicode_test_cases {
            ...
            assert!(
                result.is_ok(),
                "Failed to normalize Unicode path: {}",
                input
            );
            assert_eq!(
                result.unwrap(),
                expected,
                "Unicode path normalization mismatch for: {}",
                input
            );
        }
        ...
        match nfd_result {
            Ok(canonical) => {
                ...
                if canonical == "c:/users/café/media" {
                    ...
                    println!(
                        "  Note: Unicode normalization (NFD -> NFC) may not be fully implemented"
                    );
            ...
            Err(e) => {
                println!(
                    "NFD Unicode path normalization failed: {} -> {:?}",
                    nfd_path, e
                );
    ...
    fn test_roundtrip_conversion() {
        ...
        for canonical_path in test_paths {
            ...
            let windows_path = normalizer.canonical_to_platform(canonical_path);
            assert!(
                windows_path.is_ok(),
                "Failed to convert canonical to Windows: {}",
                canonical_path
            );
            ...
            assert!(
                back_to_canonical.is_ok(),
                "Failed to convert Windows back to canonical: {:?}",
                windows_pathbuf
            );

            // Should match original canonical path
            assert_eq!(
                back_to_canonical.unwrap(),
                canonical_path,
    ...
    fn test_invalid_path_handling() {
        ...
        for invalid_path in invalid_paths {
            ...
            if let Ok(canonical) = result {
                println!(
                    "Path was normalized (might be acceptable): {} -> {}",
                    invalid_path, canonical
                );
            } else {
                println!("Path correctly rejected: {}", invalid_path);
            }
        }
        ...
        if long_path_result.is_ok() {
            println!(
                "Long path was normalized (might be acceptable): {} chars",
                long_path.len()
            );
        } else {
    ...
    fn test_platform_path_normalizer_creation() {
        ...
        let test_paths = vec!["C:/Test/Path", "/home/user/media", "relative/path"];
        ...
        for path_str in test_paths {
            ...
            match result {
                ...
                Err(e) => {
                    println!(
                        "Path normalization failed (may be expected): {} -> {:?}",
                        path_str, e
                    );
                }
            }
...
mod filesystem_integration_tests {
    ...
    async fn test_symbolic_link_resolution() {
        ...
        #[cfg(target_os = "windows")]
        {
                ...
                match canonical_result {
                    Ok(canonical_path) => {
                        ...
                        let original_canonical =
                            fs_manager.get_canonical_path(&original_file).unwrap();
            ...
            if symlink_result.is_ok() {
                test_symlink_resolution(fs_manager.as_ref(), &symlink_file, &original_file).await;
            } else {
                println!(
                    "Symbolic link creation failed (expected without admin privileges): {:?}",
                    symlink_result.err()
                );
            }
        }
        ...
        match junction_result {
            Ok(()) => {
                ...
                for (i, original_file) in media_files.iter().enumerate() {
                    ...
                    let junction_canonical_result =
                        fs_manager.canonicalize_path(&junction_file).await;

                    match junction_canonical_result {
                        Ok(junction_canonical) => {
                            println!(
                                "File {}: Original: {} -> Junction: {}",
                                i + 1,
                                original_canonical,
                                junction_canonical
                            );
                            ...
                            assert_eq!(
                                original_canonical, junction_canonical,
    ...
    async fn test_symlink_resolution(
        fs_manager: &dyn FileSystemManager,
        symlink_path: &Path,
        original_path: &Path,
    ) {
            ...
            match canonical_result {
                Ok(canonical_path) => {
                    ...
                    if canonical_path.len() > 2 {
                        assert!(
                            canonical_path.contains("/") || canonical_path.starts_with("\\\\"),
                            "Canonical path should use forward slashes: {}",
                            canonical_path
                        );
                    }
            ...
            Ok(canonical_path) => {
                ...
                assert!(
                    canonical_path.contains("level1/level2/level3/level4/level5")
                        || canonical_path.contains("Level1/Level2/Level3/Level4/Level5"),
                    "Deep path structure should be preserved: {}",
                    canonical_path
                );
            }
            Err(e) => {
            ...
            match fs::write(&file_path, b"test content") {
                Ok(()) => {
                    ...
                    match canonical_result {
                        Ok(canonical_path) => {
                            println!(
                                "Special character file normalized: {} -> {}",
                        ...
                        Err(e) => {
                            println!(
                                "Failed to normalize special character file {}: {}",
                                filename, e
                            );
                        }
                    }
                ...
                Err(e) => {
                    println!(
                        "Could not create file with special characters {}: {}",
                        filename, e
                    );
                }
            }
...
mod additional_integration_tests {
    ...
    async fn test_case_sensitivity_edge_cases() {
        ...
        for (file_path, filename, _content) in &created_files {
            ...
            match canonical_result {
                Ok(canonical_path) => {
                    ...
                    assert!(
                        canonical_lower.contains("testvideo"),
        ...
        if created_files.len() > 1 {
            ...
            if canonical_paths.len() > 1 {
                ...
                let all_same = canonical_paths
                    .iter()
                    .all(|(canonical, _)| canonical == first_canonical);

                if all_same {
                    println!(
                        "All case variations normalized to same path (case-insensitive filesystem)"
                    );
                } else {
                    println!(
                        "Case variations normalized to different paths (case-sensitive filesystem)"
                    );
    ...
    async fn test_relative_path_normalization() {
        ...
        match relative_canonical_result {
            ...
            Err(e) => {
                println!(
                    "Relative path normalization failed (may be expected): {}",
                    e
                );
        ...
        match parent_canonical_result {
            Ok(parent_canonical) => {
                ...
                if parent_canonical.contains("..") {
                    println!(
                        "  Note: .. components not resolved in canonical path (may be expected)"
                    );
                } else {
    ...
    async fn test_path_normalization_performance() {
        ...
        println!(
            "Normalized {} files in {:?} ({:.2} files/ms)",
        ...
        assert!(
            successful_normalizations >= file_count * 9 / 10,
