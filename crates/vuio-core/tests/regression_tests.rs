...
use vuio_core::platform::filesystem::{
    create_platform_path_normalizer, PathNormalizer, WindowsPathNormalizer,
};
...
mod critical_path_scenarios {
    ...
    fn test_mixed_case_paths_windows() {
        ...
        for (path_variations, expected_canonical) in mixed_case_scenarios {
            ...
            for path_str in &path_variations {
                ...
                assert!(
                    result.is_ok(),
                    "Failed to normalize mixed case path: {}",
                    path_str
                );
        ...
        for (unc_variations, expected_canonical) in unc_test_scenarios {
            for path_str in &unc_variations {
                ...
                assert!(
                    ...
                    "UNC canonical format should start with //: {}",
                    canonical
                );
            }
        }
    ...
    fn test_very_long_path_names() {
        ...
        let long_path_tests = vec![
            ...
            // UNC path with long server and share names
            {
                ...
                let expected = format!(
                    "//{}/{}/movies/longmoviename.mp4",
                    long_server.to_lowercase(),
                    long_share.to_lowercase()
                );
                (path, expected)
            },
        ];
            ...
            match result {
                Ok(canonical) => {
                    ...
                    assert!(
                        canonical
                            .chars()
                            .all(|c| c.is_lowercase() || !c.is_alphabetic() || c == '/'),
                        "Long path canonical should be lowercase: {}",
                        canonical
                    );
                }
                Err(e) => {
                    ...
                    println!(
                        "Long path rejected (may be acceptable): {} -> {:?}",
                        long_path, e
                    );
                    ...
                    if long_path.len() < 1000 {
                        panic!(
                            "Reasonable length path should not be rejected: {} ({})",
                            long_path, e
                        );
                    }
                }
    ...
    fn test_special_characters_in_paths() {
        ...
        let special_char_tests = vec![
            // Unicode characters
            (
                r"C:\Users\José\Música\Canción.mp3",
                "c:/users/josé/música/canción.mp3",
            ),
            (
                r"C:\Users\François\Vidéos\Film.mp4",
                "c:/users/françois/vidéos/film.mp4",
            ),
            (
                r"C:\Users\张三\媒体\视频.mp4",
                "c:/users/张三/媒体/视频.mp4",
            ),
            (
                r"C:\Users\محمد\وسائط\فيديو.mp4",
                "c:/users/محمد/وسائط/فيديو.mp4",
            ),
            // Emoji and special Unicode
            (
                r"C:\Media\🎵Music\🎤Artist\Song.mp3",
                "c:/media/🎵music/🎤artist/song.mp3",
            ),
            (
                r"C:\Media\📹Videos\🎬Movies\Film.mp4",
                "c:/media/📹videos/🎬movies/film.mp4",
            ),
        ...
        for (input_path, expected_canonical) in special_char_tests {
            ...
            match result {
                Ok(canonical) => {
                    ...
                    let back_to_windows = normalizer.canonical_to_platform(&canonical);
                    match back_to_windows {
                        ...
                        Err(e) => {
                            println!(
                                "Roundtrip conversion failed (may be acceptable): {} -> {:?}",
                                canonical, e
                            );
                        }
                    }
                ...
                Err(e) => {
                    println!(
                        "Special character path normalization failed: {} -> {:?}",
                        input_path, e
                    );
                    ...
                    if input_path
                        .chars()
                        .all(|c| c.is_ascii() || c == 'é' || c == 'ñ' || c == 'ü')
                    {
                        panic!(
                            "Basic Unicode characters should be supported: {} ({})",
                            input_path, e
                        );
                    }
                }
    ...
    fn test_unicode_normalization_forms() {
        ...
        let unicode_normalization_tests = vec![
            ...
            (
                r"C:\Documents\résumé.pdf",
                r"C:\Documents\re\u{0301}sume\u{0301}.pdf",
            ),
...
mod performance_regression_tests {
    ...
    fn test_path_normalization_performance() {
        ...
        for path_str in &test_paths {
            ...
            assert!(
                result.is_ok(),
                "Path normalization should succeed: {}",
                path_str
            );
        }
        ...
        println!(
            "Path normalization performance: {:.0} paths/second",
            paths_per_second
        );

        // Should be able to normalize at least 10,000 paths per second
        assert!(
            paths_per_second > 10000.0,
