...
async fn handle_browse_metadata<D: DatabaseManager + 'static>(
    params: &BrowseParams,
    state: &AppState<D>,
) -> Response {
...
async fn resolve_container_metadata<D: DatabaseManager + 'static>(
    object_id: &str,
    state: &AppState<D>,
) -> (String, String, usize) {
    if object_id == "0" {
...
async fn count_media_folder_children<D: DatabaseManager + 'static>(
    state: &AppState<D>,
    media_type_filter: &str,
    path_prefix_str: &str,
) -> usize {
