...
pub enum WindowsError {
    ...
    #[error(
        "Windows service registration failed: {reason}. Try running with administrator privileges"
    )]
...
pub enum MacOSError {
    ...
    #[error(
        "macOS Application Firewall is blocking connections. Add VuIO to allowed applications"
    )]
...
pub enum LinuxError {
    ...
    #[error(
        "SELinux policy violation: {context}. Try: sudo setsebool -P httpd_can_network_connect 1"
    )]
...
impl PlatformError {
    ...
    pub fn user_message(&self) -> String {
        match self {
            PlatformError::Windows(err) => format!(
                "Windows Error: {}\n\nTroubleshooting: {}",
                err,
                err.troubleshooting_guide()
            ),
            PlatformError::MacOS(err) => format!(
                "macOS Error: {}\n\nTroubleshooting: {}",
                err,
                err.troubleshooting_guide()
            ),
            PlatformError::Linux(err) => format!(
                "Linux Error: {}\n\nTroubleshooting: {}",
                err,
                err.troubleshooting_guide()
            ),
            PlatformError::Bsd(err) => format!(
                "BSD Error: {}\n\nTroubleshooting: {}",
                err,
                err.troubleshooting_guide()
            ),
            PlatformError::Database(err) => format!(
                "Database Error: {}\n\nRecovery: {}",
                err,
                err.recovery_strategy()
            ),
            PlatformError::Configuration(err) => format!(
                "Configuration Error: {}\n\nSolution: {}",
                err,
                err.solution_guide()
            ),
...
impl DatabaseError {
    pub fn recovery_strategy(&self) -> &'static str {
        match self {
            ...
            DatabaseError::RestoreFailed { .. } => "Fallback to fresh database creation",
            DatabaseError::QueryFailed { .. } => "Retry with simplified query or rebuild database",
        }
    }
