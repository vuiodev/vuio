...
**Audio and mirroring receivers** — most third-party AirPlay 2 TVs, including Sony Android TV — advertise only bit 49 (`SupportsAirPlayVideoV2`) and expose no URL-video endpoint at all. VuIO does not list them as AirPlay renderers by default, because the cast cannot succeed. Set `VUIO_AIRPLAY_ALLOW_V2_ONLY=1` to list them anyway and have the cast report exactly what the receiver answered.
...
VuIO streams audio to these receivers. AirPlay never carries MP3 — its `audioFormat` bitmask covers only PCM, ALAC, AAC-LC, AAC-ELD and OPUS — so, exactly like an iPhone, VuIO decodes the source itself and pushes `PCM/44100/16/2` (`0x800`). MP3, M4A/AAC, FLAC, WAV, ALAC and Vorbis are decoded with Symphonia, resampled to 44.1 kHz stereo, framed into 352-sample RTP packets, encrypted with ChaCha20-Poly1305, and paced in real time against an NTP timing server with periodic clock-sync packets on the control channel. Audio therefore works on receivers that cannot play video at all, and the renderer list reports `video: false, audio: true` for them.
